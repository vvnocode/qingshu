+++
date = '2026-03-15T11:40:00+08:00'
draft = false
title = 'OpenClaw 规范与安全准则'
tags = ['OpenClaw', 'AI', 'Agent', '安全']
+++

# OpenClaw 规范与安全准则

无论你是个人使用还是团队部署，本篇的规范都**必须遵守**。内容涵盖 AGENTS.md 行为模板、安全纵深防御配置和 Denied Agent 兜底拒绝。
...
>
> 模板中的 Memory Discipline 规则详解参见 [05-Memory：持久记忆系统](./05-OpenClaw%20Memory：让%20AI%20越用越聪明.md) 第 4 节。
...
当需要将 Gateway 开放给内网其他设备访问时（设置 `bind: "lan"`），必须额外注意以下安全事项。具体的局域网访问配置步骤详见 [03-核心概念与配置](./03-OpenClaw%20核心概念与配置.md) 第 7.2 节，本节补充安全层面的注意事项：

| 安全要求 | 配置方式 | 说明 |
|---------|---------|------|
| **必须开启 Token 认证** | `auth.mode: "token"` | 局域网中仍有未授权设备，Token 是第一道防线 |
| **严格配置 CORS 白名单** | `controlUi.allowedOrigins` | 只填写实际需要访问的来源 IP/端口，不要使用通配符 `*` |
| **不对公网开放** | `bind: "lan"` | 仅绑定内网网卡，若需跨网段访问请通过 VPN |
| **定期轮换 Token** | — | 建议每 90 天或成员变动时轮换，见第 2.8 节 |

**`dangerouslyDisableDeviceAuth` 风险说明：**
...
该字段设为 `true` 时，访问 Dashboard Web UI **不再需要走设备配对流程**——任何能访问该端口的设备持有 Token 即可打开控制界面。

| 场景 | 建议值 | 说明 |
|------|--------|------|
| 主动调试阶段（临时、可控） | `true` | 仅在明确需要调试时开启，调试结束**立即关闭** |
| 所有其他情况（包括内网部署） | **`false`（默认）** | 官方建议：*"keep it off unless you are actively debugging"* |

> ⚠️ **官方定性为高危配置**：`dangerouslyDisableDeviceAuth: true` 完全绕过设备身份验证——任何持有 Token 的设备均可访问 Control UI，包括中间人攻击者。字段名中的 `dangerously` 是刻意的警示。**只在主动调试时短暂开启，调试结束必须立即恢复为 `false`，不建议在任何持久部署中保留此设置。**
